import { CheckCircle2, Circle, Calendar, Clock, AlertTriangle } from 'lucide-react';
import { usePortfolioStore } from '../../store/usePortfolioStore';
import { useState } from 'react';

export default function SubmissionList() {
  const { targetProgram, generateSubmissionList } = usePortfolioStore();
  const [items, setItems] = useState(generateSubmissionList());
  ...
  const getDeadlineStatus = (deadline: string) => {
    if (!deadline || deadline === '-') return { status: 'none', label: '无截止日期' };
  ...
  return (
    <div className="card p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          onClick={() => setItems(generateSubmissionList())}
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="ml-auto text-right">
              <p className={`text-sm font-medium ${
                getDeadlineStatus(targetProgram.deadline).status === 'overdue' ? 'text-red-600' :
                getDeadlineStatus(targetProgram.deadline).status === 'urgent' ? 'text-orange-600' :
                getDeadlineStatus(targetProgram.deadline).status === 'soon' ? 'text-amber-600' :
                'text-green-600'
              }`}>
          ...
          <div key={category} className="border border-zinc-200 rounded-lg overflow-hidden">
            ...
            <div className="divide-y divide-zinc-100">
              {categoryItems.map((item) => {
                ...
                return (
                  <div key={item.id} className="px-4 py-3 hover:bg-zinc-50 transition-colors">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-4">
                          <p className={`font-medium ${item.completed ? 'text-zinc-400 line-through' : 'text-zinc-800'}`}>
                            {item.item}
                          </p>
                          {item.deadline && item.deadline !== '-' && (
                            <div className={`flex items-center gap-1 flex-shrink-0 text-xs ${
                              ...
                              deadlineStatus.status === 'soon' ? 'text-amber-600' :
                              'text-green-600'
                            }`}>
